...
use crate::{
  generated::{
    api::{HoursBucket, LeaderboardEntry},
...
pub struct LeaderboardConfig {
  pub show_overtime: bool,
  pub member_types: Vec<i32>,
}
...
/// Combine overtime into regular for a single bucket.
fn combine_overtime(bucket: &mut HoursBucket) {
  bucket.regular_secs += bucket.overtime_secs;
  bucket.overtime_secs = 0.0;
}

/// Compute the leaderboard entries, filtered and configured by `LeaderboardConfig`.
pub fn compute_leaderboard(config: &LeaderboardConfig) -> Result<Vec<LeaderboardEntry>> {
  ...
  let mut entries: Vec<LeaderboardEntry> = accumulators
    ...
    .filter_map(|(member_id, mut acc)| {
      ...
      // Filter by member type if configured
      if !config.member_types.is_empty() && !config.member_types.contains(&member.member_type) {
        return None;
      }

      // Combine overtime into regular if not showing overtime separately
      if !config.show_overtime {
